...
export function Sidebar() {
  ...
  return (
    <>
      ...
      {/* 桌面端侧边栏 (lg及以上显示) */}
      {/* ===================== */}
      <aside className="hidden lg:flex w-64 h-screen bg-white/80 backdrop-blur-xl border-r border-orange-200/50 flex-col fixed left-0 top-0 shadow-[4px_0_24px_rgba(249,115,22,0.05)] z-40">
      ...
      {/* 手机端顶部栏 (lg以下显示) */}
      {/* ===================== */}
      <header className="lg:hidden fixed top-0 left-0 right-0 z-40 h-14 bg-white/90 backdrop-blur-xl border-b border-orange-200/50 flex items-center justify-between px-4 shadow-sm">
        <Link href="/dashboard" className="flex items-center gap-2">
          ...
          <span className="font-extrabold text-lg text-orange-950 tracking-tight">UniFlow</span>
        </Link>
        <button
          onClick={handleLogout}
      ...
      {/* 手机端底部导航栏 (lg以下显示) */}
      {/* ===================== */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/90 backdrop-blur-xl border-t border-orange-200/50 flex items-center justify-around px-2 py-2 shadow-[0_-4px_24px_rgba(249,115,22,0.08)]">
        {NAV_ITEMS.map((item) => {
          const isActive = pathname === item.href || pathname.startsWith(item.href + "/");
          const Icon = item.icon;
          return (
